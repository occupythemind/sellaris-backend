...
class Category(models.Model):
    ...
    name = models.CharField(
        max_length=70, 
        help_text="e.g., Laptop, Mobile Phones"
    )
...
class Product(models.Model):
    ...
    name = models.CharField(
        max_length=70, 
        help_text="e.g., Samsung, IPhone"
    )

    brand = models.CharField(
        max_length=70, 
        help_text="e.g., Apple Inc"
    )
...
class ProductVariant(models.Model):
    ...
    sku_code = models.CharField(
        max_length=40, 
        unique=True, 
        help_text="Unique identifier/code for the product (SKU)", 
        blank=True
    )

    color = models.CharField(
        max_length=50, 
        help_text="The exact color e.g. Red, #ff00ff"
    )

    storage_size = models.CharField(
        max_length=20, 
        help_text="e.g., 128GB, 256GB", 
        blank=True
    )
